...
import { alpha2FromNumeric, formatLocation } from "../masternode-map/iso-codes";
import { AVAILABILITY_COLOR, peerBucket } from "./status";
import {
  type ClusterFeature,
...
const MARKER_PASSIVE_STYLE = { default: { cursor: "default" } } as const;
...
export function PeersMapCanvas({
  points,
  highlightedCountry,
  onSelectCountry,
  onSelectCluster,
        ...
        >
          ...
          <g pointerEvents="auto">
            {clusters.map((cluster) => {
              ...
              const point = cluster.properties.point as PeerGeoPoint;
              return (
                <NodeMarker
                  key={`n-${point.address}`}
                  point={point}
...
const ClusterMarker = memo(function ClusterMarker({
  ...
  const { total, available, unavailable } = props;
  const size = 18;
...
const NodeMarker = memo(function NodeMarker({
  ...
  const { k } = useZoomPanContext();
  const inv = 1 / Math.max(k, 0.001);
  const bucket = peerBucket(point.available);
  const color = AVAILABILITY_COLOR[bucket];
  const payload = useMemo(
    () =>
    ...
    >
      <g transform={`scale(${inv})`} style={MARKER_SCALE_STYLE}>
        <circle
          ...
          fill={color}
          opacity={0.18}
          className={point.available ? "animate-subtle-pulse" : undefined}
        />
      ...
      {tooltip.kind === "cluster" ? (
        <div className="flex flex-col gap-1">
          <div className="font-medium text-sm">
            {tooltip.count} peers in this region
          </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2 pt-1">
            <span
              ...
              style={{
                background:
                  AVAILABILITY_COLOR[peerBucket(tooltip.point.available)],
              }}
            />
            <span className="text-[11px] uppercase tracking-wide text-muted-foreground">
              {tooltip.point.available ? "Available" : "Unavailable"}
            </span>
          </div>
          {tooltip.point.userAgent && (
            <div className="truncate pt-1 font-mono text-[10px] text-muted-foreground/80">
              {tooltip.point.userAgent}
              {tooltip.point.protocolVersion != null &&
                ` · v${tooltip.point.protocolVersion}`}
            </div>
          )}
        </div>
      )}
